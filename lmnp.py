...
if st.button("Enregistrer et Calculer"):
    ...
    try:
        ...
        # NETTOYAGE PRÉVENTIF : Supprimer les anciens fichiers TEMP
        try:
            ...
            for file in all_files:
                if file['name'].startswith('TEMP_'):
                    try:
                        ...
                        st.write(f"🗑️ Nettoyé: {file['name']}")
                    except:
                        pass
        except:
            pass
        ...
        with st.spinner("Création de l'espace isolé..."):
            ...
            updates = [
                {'range': 'B4', 'values': [[prix_achat]]}, 
                {'range': 'B5', 'values': [[frais_notaire]]},
                {'range': 'B6', 'values': [[frais_agence]]}, 
                {'range': 'B8', 'values': [[travaux]]},
                {'range': 'B7', 'values': [[mobilier]]}, 
                {'range': 'B12', 'values': [[date_str]]},
                {'range': 'B84', 'values': [[loyer]]}, 
                {'range': 'B85', 'values': [[charges]]},
                {'range': 'B86', 'values': [[taxe_fonciere]]}, 
                {'range': 'B87', 'values': [[interets]]},
                {'range': 'B93', 'values': [[amort_excedentaires]]}, 
                {'range': 'B98', 'values': [[deficits_anterieurs]]},
        ...
        if temp_sheet_id:
            try:
                gc.del_spreadsheet(temp_sheet_id)
                st.info(f"🗑️ Fichier temporaire supprimé")
            except Exception as del_error:
                st.warning(f"⚠️ Impossible de supprimer le temp file: {del_error}")